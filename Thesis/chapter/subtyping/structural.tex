...
\begin{rules}[base:subtyping]{Subtyping for the base system}
  % Base types
  ...
  \and \infer=[\Sub\internal]{\internals{A}{I} \sub \internals{A'}{J}}
    {I \subseteq J & A_k \sub A'_\indexVar~\text{for}~\indexVar \in J}
  \and \infer=[\Sub\lolli]{A \lolli B \sub A' \lolli B'}
    {A' \sub A & B \sub B'}
  \and \infer=[\Sub\external]{\externals{A}{I} \sub \externals{A'}{J}}
    {J \subseteq I & A_k \sub A'_\indexVar~\text{for}~\indexVar \in J}
  % Recursive types
