...
\end{theorem}
\begin{proof}
  Follows from a simple coinduction for reflexivity\todo{There is a problem here. Related to the same problem for equality.}. For transitivity, we use a lexicographic combination of coinduction on the two subtyping derivations and induction on $\size{(B)}$. \todo{Does this sentence read right?} We omit details here since the proof is fairly standard (except for the use of coinduction) and since we will be changing the subtyping relation in \cref{refinements:subtyping}.
\end{proof}
