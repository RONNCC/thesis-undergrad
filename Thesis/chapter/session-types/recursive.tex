...
\begin{proof}
  Reflexivity \todo{Ask Frank: what does reflexivity follow from? Is it induction or coinduction on the type?} Symmetry follow from a simple coinduction on the derivation of type equality using the symmetric rules for $\TypeEq{\mu\Right}$ and $\TypeEq{\mu\Left}$. For transitivity, we use a lexicographical coinduction on the two equality derivations and induction on $\size{(B)}$.
