...
\lstset{language=krill, style=custom}
...
\footnote{Perhaps the only thing worth noting is that we write \lstinline!`<channel> $\leftarrow$ <process-name> = <body>! to introduce a recursive process where \lstinline!`<channel>! labels the channel the process \emph{provides}, and \lstinline!<body>! can refer to \lstinline!<process-name>! (see \cref{recursive}).}
...
Padovani describes a calculus very similar to ours~\cite{Padovani10} where he interprets internal and external choices using intersections and union. This is quite similar to what we did in \cref{reinterpreting-choice} except we keep singleton choices at the type and term levels to maintain the connection to linear logic. Padovani is able to remove them completely since his calculus is based on \textsc{CSS}~\cite{NicolaH87}. While we give axiomatic rules for (sub)typing, he takes a semantic approach. We believe this is complementary to our work, though it makes deriving algorithmic rules harder (in fact, Padovani leaves this as future work). More significantly, Padovani does not consider higher-order types and processes, which means it is not possible to communicate channels along channels.
