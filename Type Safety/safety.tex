...
\section{Restricted Typing Rules}

The typing rules given above are natural in that they mirror the logical rules and match our expectations. However, attempting a proof of type preservation reveals that they are too liberal to use in practice since they lead to many cases and sub-cases. Instead, we restrict the places where non-structural rules can be applied in order to force a ``normal-form'' on typing derivations. We show that the new rules are sound and complete with respect to the old ones, and proceed with the limited rules form now on.

The general idea is to restrict the uses of non-structural rules just before communicating on the relevant channels. We first define an auxiliary judgment $\primed{P}{c}$ intended mean that the next action of process $P$ will be to communicate along channel $c.$

\begin{mathpar}
  \infer{\primed{\tclose c}{c}}{}
  \and\infer{\primed{ \twait c P }{c}}{}
  \and\infer{\primed{ \tsend{c}{y}{P_y}{Q} }{c}}{}
  \and\infer{\primed{ \trecv{x}{c}{R_x} }{c}}{}
  \and\infer{\primed{ \tselect{c}{}{P} }{c}}{}
  \and\infer{\primed{ \tcase c {\tbranches Q i} }{c}}{}
\end{mathpar}

The structural rules ($\terminate\Right, \terminate\Left, \tensor\Right, \tensor\Left, \internal\Right, \internal\Left, \lolli\Right, \lolli\Left, \external\Right, \external\Left$) remain unchanged. It turns out to be convenient if forwarding and cut are not primed on any channels, so we will need to handle those rules separately. The restricted typing judgment $\typesR{\ctx}{P}{c}{A}$ is given by the following rules:

\begin{mathpar}
  \infer[\irb{Sub}\Right]{\typesR{\ctx}{P}{c}{A'}}
    { \typesR{\ctx}{P}{c}{A}
    & \primed P c
    & A \sub A'
    }
  \and \infer[\irb{Sub}\Left]{\typesR{\ctx, c : A}{P}{d}{B}}
    {\typesR{\ctx, c : A'}{P}{d}{B}
    & \primed P c
    & A \sub A'
    }
...
\begin{mathpar}
  ...
  % Terminate
  \and \infer[\terminate\Right]{\syn{\emptyCtx}{\tclose c}{c}{\terminate}}
    {}
  \and \infer[\terminate\Left]{\typeS{\ctx, c : \terminate}{\twait c P}{d}{A}}
    {\typeSJ{P}{d}{A}}
  % Tensor
