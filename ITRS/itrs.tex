...
% When types are interpreted as sets, intersection and union are set theoretic intersection and union. \todo{I don't think I need the set theoretic interpretation, but there are many ways to think about intersections and unions. Which ones are important?}
\todo{Delete set-theoretic interpretation because it is tricky here due to the higher-order nature of types.  Better not to get into it---the behavioral characterization is what we want. -fp}
...
\todo{The whole section is copy-pasted from the thesis. Shorten and reword.}
...
A type is then said to be contractive if every occurrence of $\recursive t A$ satisfies $t \not\in \unguarded{(A)}$ as formalized in Figure \ref{contractiveness}. \todo{I think the figure is unnecessary in a paper.}
...
\todo{Can be summarized in two sentences. Don't need a whole section with two paragraphs.}
